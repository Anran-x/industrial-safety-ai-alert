...
| 模块 | 方案 | 状态 |
|---|---|---|
| 安全帽佩戴检测 | YOLO11s 微调(SHWD, 2 类),imgsz 960 | ✔ 完成(test mAP50 97.7%;全量 706 张验证:0.35 阈值召回 96.1%) |
...
| 演示端 | Gradio Web 三页签:视频检测 / 告警中心(截图证据墙+历史记录) / 使用说明,高级参数默认展开 | ✔ 完成(~15 FPS@960,头盔推理 47ms/帧) |
...
2. **为什么用规则法做倒地/闯入而非时序模型**:小样本下规则法可解释、可调参、
   ...
   (宽高比 > 阈值 + 躯干水平判定 + 连续帧确认)。诚实边界:规则阈值基于平视
   (cam0)数据标定,正俯视(天花板)场景规则法不适用(召回上限 ~30%,无参数甜区);
   工业部署建议 45° 斜装视角——已用 Le2i 帧段端到端验证存在参数甜区
   (aspect 1.2/6 帧:Fall 57%、Likefall 假倒误报 0%),落地为 UI「视角预设」,
   详见 `docs/eval_report.md` §2.1。
...
6. **数据驱动的推理分辨率取舍**:全量 706 张/2722 目标验证后,拒绝"降阈提召回"路线
   (0.25-0.35 置信度档误报率 67%),改为 imgsz 640→960(召回 94.8%→96.1%,
   戴帽类混淆减半),阈值保持 0.35,CLIP 复核区间 [0.35,0.55] 兜底。
...
# 5. (可选)生成演示素材与样例报告
python scripts/prepare_demo_assets.py     # 生成 data/demo/*.mp4(H.264 可直接预览)
python scripts/gen_sample_report.py       # 重新生成 results/sample_report.md(三类告警样例)

# 6. 演示
python -m src.app                         # http://127.0.0.1:7860
```

> **H.264 视频输出(浏览器可预览)**:需将 `openh264-2.5.0-win64.dll` 放入
> `.venv\Scripts\` 目录(下载:https://ciscobinary.openh264.org/openh264-2.5.0-win64.dll.bz2,
> 解压后用即可);缺失时自动降级 mp4v(浏览器无法预览,可下载观看)。
...
- [x] 安全帽模型训练与评估(test mAP50 97.7%)
- [x] 倒地 / 闯入 / CLIP 兜底评估报告(UR Fall 召回 80%/误报 10%;CLIP 修正 27.3%)
- [x] 演示端 UI 分模块重构(Gradio:检测/告警图片墙/使用说明;H.264 输出)
- [x] ONNX 导出验证(fp32 精度对齐;INT8 体积 -4x,精度 -0.4pp)
- [x] 演示素材 H.264 化 + 等比缩放修复(拉伸变形曾致检出 47→62;高级参数面板默认展开)
- [x] 全量量化验证(706 张 test/2722 目标:0.35 阈值召回 94.8%,硬漏 1.8%,FP 率 5.3%)
- [x] 推理分辨率 640→960(全量召回 94.8%→96.1%,戴帽类混淆 13→7 减半,演示视频 65/66)
- [ ] 演示录屏 / 架构图 / 开源发布(可选)
