...
| 模块 | 方案 | 状态 |
|---|---|---|
| 安全帽佩戴检测 | YOLO11s 微调(SHWD, 2 类) | 训练中 → 待评估 |
| 人员倒地检测 | YOLO11s-pose 关键点 + 时序规则(多帧确认 + 复位) | 代码完成 → 待评估 |
| 危险区域闯入 | ByteTrack 多目标跟踪 + 归一化 ROI 多边形判定 | 代码完成 → 待评估 |
| 低置信度兜底复核 | CLIP 零样本语义复核(图文对齐) | 代码完成 → 待评估 |
| 告警报告 | 结构化模板报告(时间/地点/置信度/截图/建议动作),预留 VLM 接口 | 完成 |
| 告警存档 | SQLite 存储 + CSV 导出 + 截图留证 | 完成 |
| 演示端 | Gradio Web(上传视频 → 检测 → 告警面板) | 完成 |
