"""
CLIP 零样本兜底复核:对低置信度的头盔检测结果进行语义二次确认。
...
from src.config import HELMET_CLIP_LOW, HELMET_CLIP_HIGH
...
class ClipVerifier:
    ...
    def __init__(self, model_name: str = "ViT-B/32", device: Optional[str] = None,
        ...
        texts = self.PROMPTS_NO_HELMET + self.PROMPTS_HELMET
    ...
    def verify(self, image: np.ndarray, box, conf: float) -> Tuple[bool, float]:
        ...
        inp = self.preprocess(cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)).unsqueeze(0).to(self.device)
        with torch.no_grad():
            img_feat = self.model.encode_image(inp)
